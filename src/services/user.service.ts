...
export const me = expressAsyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (redisInfo) {
          return sendResponse(res, 200, true, "User info", null,{user:redisInfo});
      }
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (newEmail === config.admin_email) {
        return next(
          createHttpError(400, "You do not have permission to update this email")
        );
      }
      ...
      const newData: IChangeEmail = {
        id: userId.toString(),
        email: newEmail,
        emailActivateCode: String(newCode),
      };
      ...
      return sendResponse(
        ...
        { emailToken: token, emailCode: newCode }
      );
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        if (error.name === 'TokenExpiredError') {
          return next(createHttpError(400, "Email verification token has expired"));
        } else {
          return next(createHttpError(400, "Invalid email token"));
        }
      }
